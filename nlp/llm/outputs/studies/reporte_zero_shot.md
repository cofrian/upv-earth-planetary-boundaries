# Reporte de Evaluación: Qwen 2.5 (14B) vs Validación Humana

## Resumen Ejecutivo
- **Documentos analizados**: 150
- **Exact Match (Perfecto)**: 54.0%
- **Top-1 Accuracy**: 63.3%
- **Hit@K (Acierto Flexible)**: 56.6%
- **True Negative**: 84.3%
- **Jaccard Similarity Promedio**: 0.476
- **Hamming Loss Promedio**: 0.064
- **Rigorousness**: 84.3%
...
```
        ...
        None       0.56      0.84      0.67        51
         PB1       0.58      0.46      0.51        24
         PB2       0.75      0.75      0.75         8
         PB3       0.60      0.38      0.46         8
         PB4       0.71      0.50      0.59        10
         PB5       1.00      0.50      0.67         4
         PB6       0.50      0.50      0.50         4
         PB7       1.00      0.23      0.38        13
         PB8       1.00      0.57      0.73         7
         PB9       0.76      0.76      0.76        21

    accuracy                           0.63       150
   macro avg       0.75      0.55      0.60       150
...
- Promedio PBs asignados por Qwen: 0.55
- Máximo PBs humano: 2
- Máximo PBs Qwen: 3
- Documentos sin PB humano: 51
- Documentos sin PB Qwen: 77
...
- Valor actual: 54.0%. Esto sugiere que más de la mitad de los documentos tienen una correspondencia completa entre humano y Qwen.
...
- Evalúa si Qwen identifica correctamente el PB primario humano, independientemente de los secundarios.
- Valor actual: 63.3%.
- Interpretación: una tasa de 63.3% indica que Qwen acierta el tema principal en casi dos tercios de los casos, pero todavía hay margen de mejora.

### 3. Hit@K (Acierto Flexible)
- Comprueba si el PB primario humano aparece en la lista completa de PBs predichos por Qwen.
- Valor actual: 56.6%.
...
- Valor actual: 84.3%.
- Interpretación: un 84.3% es un buen indicador de que Qwen no sufre un ruido excesivo de positivismo en los ejemplos sin PB.

### 5. Jaccard Similarity
- Mide el solapamiento entre las listas de PBs humano y modelo en cada documento.
- Valor actual: 0.476.
- Interpretación: 0.476 indica un solapamiento moderado. Hay un número relevante de casos en los que las listas comparten al menos una etiqueta, pero no coinciden completamente.

### 6. Hamming Loss
- Mide la fracción de etiquetas PB mal clasificadas en el conjunto de 9 posibles.
- Valor actual: 0.064.
- Interpretación: 0.064 es una cifra baja, lo que sugiere que el modelo no comete errores masivos en la mayoría de las etiquetas. Sin embargo, la métrica no distingue entre falsos positivos y falsos negativos.

### 7. Rigorousness (Tasa de Filtrado)
- Mide qué porcentaje de documentos sin PB humano fueron correctamente rechazados por Qwen.
- Valor actual: 84.3%.
- Interpretación: una tasa alta como 84.3% indica que el modelo es bastante conservador frente a documentos sin PB, lo cual es positivo para evitar ruido.

### 8. Positivity Bias (Alucinación Verde)
- Indica qué porcentaje de documentos ‘None’ humanos recibió al menos un PB predicho por Qwen.
- Valor actual: 15.7%.
- Interpretación: el 15.7% revela un sesgo de positivismo presente, pero no excesivo. El objetivo de la evaluación debe ser reducir esta cifra sin sacrificar demasiada precisión primaria.
...
- Compara la distribución del número de PBs asignados por humano y por Qwen.
- Un violín más estrecho en Qwen sugiere que el modelo es más conservador.
...
### 5. Distribución de Predicciones de Qwen
...
- Si Qwen asigna menos secundarios y aún mantiene buen Top-1, su comportamiento es conservador pero más preciso.
...
- El modelo parece ser relativamente conservador: la baja Hamming Loss y la alta Rigorousness indican que evita asignar demasiadas PBs, pero el Positivity Bias del 15.7% muestra que aún produce falsos positivos en algunos documentos ‘None’.
- El hecho de que el True Negative sea alto (84.3%) y el Top-1 no supere el 70% sugiere un equilibrio entre precaución y falta de confianza. El modelo sabe cuándo no asignar nada, pero a veces pierde el PB primario correcto.
- La Jaccard media de 0.476 indica que cuando el modelo no acierta exactamente, todavía captura parte del conjunto de PBs correcto. Esto es consistente con un razonamiento que identifica el dominio general del texto pero falla en ordenar o jerarquizar algunos PBs secundarios.
...
2. Hay margen para afinar la respuesta ‘None’: el modelo ya es bastante bueno rechazando casos sin PB, pero debe reducir el 15.7% de alucinación verde.
...
## Gráficos Generados
1. [Matriz de Confusión Normalizada](1_matriz_confusion_qwen.png)
2. [Análisis de Overassignment (Violin Plot)](2_overassignment_qwen.png)
3. [Tasa de Acierto por Planetary Boundary](3_accuracy_por_pb_qwen.png)
4. [Sesgo de Positividad](4_sesgo_positividad_qwen.png)
5. [Distribución de Predicciones de Qwen](5_distribucion_predicciones_qwen.png)
...
---
*Generado automáticamente el 2026-05-05 18:08:16*
